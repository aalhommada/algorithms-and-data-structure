...
- **Space** — three integer pointers. Sorting the concatenation instead (`nums1[m:] = nums2; nums1.sort()`) is a valid one-liner but costs `O((m+n) log(m+n))` time, which throws away the sorted structure both inputs already have. The linear in-place merge is the point of the problem.
