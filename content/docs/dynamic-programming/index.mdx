...
| [Min Cost Climbing Stairs](/docs/dynamic-programming/min-cost-climbing-stairs) | Easy | `f(i) = cost[i] + min(f(i-1), f(i-2))` |
| [House Robber](/docs/dynamic-programming/house-robber) | Medium | `f(i) = max(f(i-1), f(i-2) + nums[i])` |
| [Coin Change](/docs/dynamic-programming/coin-change) | Medium | `f(a) = 1 + min(f(a - c)) over coins c` |
| [Longest Increasing Subsequence](/docs/dynamic-programming/longest-increasing-subsequence) | Medium | `f(i) = 1 + max(f(j)) where nums[j] < nums[i]` |
| [Unique Paths](/docs/dynamic-programming/unique-paths) | Medium | `f(r,c) = f(r-1,c) + f(r,c-1)` |
| [Longest Common Subsequence](/docs/dynamic-programming/longest-common-subsequence) | Medium | `f(i,j) = f(i-1,j-1)+1` or `max(f(i-1,j), f(i,j-1))` |
| [Word Break](/docs/dynamic-programming/word-break) | Medium | `ok(i) = any(ok(j) and s[j:i] in words)` |

<Callout type="info" title="One dimension or two">
